...
object AISframe
{
	...
	def main(args: Array[String])
	{
		...
		val ar = arr_MMSI_time.map(_.split(",")).map(x=>((x(0),x(1)),List(x(1),x(2)))).groupByKey().map(x=>(x._1,x._2.toList))
		val de = dep_MMSI_time.map(_.split(",")).map(x=>((x(0),x(1)),List(x(1),x(2)))).groupByKey().map(x=>(x._1,x._2.toList))
