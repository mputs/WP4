...
object AISframe
{
	...
	def main(args: Array[String])
	{
		...
		val data = sc.textFile(rawdatafile).map(_.split(","))
			...
			.map((x._1.productIterator.toArray++x._2.productIterator.toArray).map(x=> x.toString))
			.mapPartitions{it => 
		...
		val ship_orig = data.map(x=>(x(0), Array(x(1).toString,x(2).toString,x(4).toString,x(8).toString,x(9).toString,x(10).toString)))
			.groupByKey()
